...
public class CartController {
    ...
    public ResponseEntity<ApiResponse<CartDto>> addItem(
        ...
        if (existingItemOpt.isPresent()) {
            ...
            // Fetch product information from Product Service via Feign
            String name = "Product #" + request.productId();
            String image = "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=500";
            BigDecimal price = new BigDecimal("49.99");

            try {
                Map<String, Object> resp = productClient.getProductById(request.productId());
                if (resp != null && resp.containsKey("data")) {
                    @SuppressWarnings("unchecked")
                    Map<String, Object> data = (Map<String, Object>) resp.get("data");
                    if (data != null) {
                        name = (String) data.getOrDefault("name", name);
                        image = (String) data.getOrDefault("imageUrl", image);
                        Object discPrice = data.get("discountPrice");
                        Object regPrice = data.get("price");
                        if (discPrice != null) {
                            price = new BigDecimal(discPrice.toString());
                        } else if (regPrice != null) {
                            price = new BigDecimal(regPrice.toString());
                        }
                    }
                }
            } catch (Exception e) {
                // If service is temporarily registering, use sensible fallback
            }
